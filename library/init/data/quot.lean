...
constant quotient (α : Sort u) [setoid α] : Sort u
protected constant quotient.mk {α : Sort u} [setoid α] : α → quotient α
protected axiom quotient.sound : ∀ {α : Sort u} [s : setoid α] {a b : α}, a ≈ b → quotient.mk a = quotient.mk b
protected constant quotient.rec {α : Sort u} [setoid α] {β : quotient α → Sort v}
    (f : ∀ a, β (quotient.mk a)) (h : ∀ a b, ∀ hs: a ≈ b, f a == f b) : ∀ q, β q

computation_rule quotient.comp_rule {α : Sort u} (a : α) [setoid α] {β : quotient α → Sort v}
...
protected def lift {α : Sort u} {β : Sort v} [s : setoid α] (f : α → β) : (∀ a b, a ≈ b → f a = f b) → quotient α → β
| h q := quotient.rec f (λ a b hs, heq_of_eq (h a b hs)) q

attribute [elab_as_eliminator]
protected lemma ind {α : Sort u} [setoid α] {β : quotient α → Prop} : (∀ a, β ⟦a⟧) → ∀ q, β q
| h q := quotient.rec h (λ a b hs, heq_of_eq_rec_left (quotient.sound hs) rfl) q

attribute [reducible, elab_as_eliminator]
protected def lift_on {α : Sort u} {β : Sort v} [setoid α] (q : quotient α) (f : α → β) (c : ∀ a b, a ≈ b → f a = f b) : β :=
quotient.lift f c q

attribute [elab_as_eliminator]
protected lemma induction_on {α : Sort u} [s : setoid α] {β : quotient α → Prop} (q : quotient α) (h : ∀ a, β ⟦a⟧) : β q :=
quotient.ind h q

lemma exists_rep {α : Sort u} [s : setoid α] (q : quotient α) : ∃ a : α, ⟦a⟧ = q :=
quotient.induction_on q (λ a, ⟨a, rfl⟩)

section
variable {α : Sort u}
variable [s : setoid α]
variable {β : quotient α → Sort v}

attribute [reducible, elab_as_eliminator]
protected def rec_on
   (q : quotient α) (f : Π a, β ⟦a⟧) (h : ∀ (a b : α) (p : a ≈ b), f a == f b) : β q :=
quotient.rec f h q

attribute [reducible, elab_as_eliminator]
protected def rec_on_subsingleton
   [h : ∀ a, subsingleton (β ⟦a⟧)] (q : quotient α) (f : Π a, β ⟦a⟧) : β q :=
...
protected def lift_on₂
  (q₁ : quotient α) (q₂ : quotient β) (f : α → β → φ) (c : ∀ a₁ a₂ b₁ b₂, a₁ ≈ b₁ → a₂ ≈ b₂ → f a₁ a₂ = f b₁ b₂) : φ :=
quotient.lift₂ f c q₁ q₂

attribute [elab_as_eliminator]
protected lemma ind₂ {φ : quotient α → quotient β → Prop} (h : ∀ a b, φ ⟦a⟧ ⟦b⟧) (q₁ : quotient α) (q₂ : quotient β) : φ q₁ q₂ :=
quotient.ind (λ a₁, quotient.ind (λ a₂, h a₁ a₂) q₂) q₁

attribute [elab_as_eliminator]
protected lemma induction_on₂
   {φ : quotient α → quotient β → Prop} (q₁ : quotient α) (q₂ : quotient β) (h : ∀ a b, φ ⟦a⟧ ⟦b⟧) : φ q₁ q₂ :=
quotient.ind (λ a₁, quotient.ind (λ a₂, h a₁ a₂) q₂) q₁

attribute [elab_as_eliminator]
protected lemma induction_on₃
   [s₃ : setoid φ]
   {δ : quotient α → quotient β → quotient φ → Prop} (q₁ : quotient α) (q₂ : quotient β) (q₃ : quotient φ) (h : ∀ a b c, δ ⟦a⟧ ⟦b⟧ ⟦c⟧)
   : δ q₁ q₂ q₃ :=
...
private def rel (q₁ q₂ : quotient α) : Prop :=
...
private lemma rel.refl : ∀ q : quotient α, q ~ q :=
λ q, quotient.induction_on q (λ a, setoid.refl a)

private lemma eq_imp_rel {q₁ q₂ : quotient α} : q₁ = q₂ → q₁ ~ q₂ :=
...
attribute [reducible, elab_as_eliminator]
protected def rec_on_subsingleton₂
   {φ : quotient α → quotient β → Sort u_c} [h : ∀ a b, subsingleton (φ ⟦a⟧ ⟦b⟧)]
   (q₁ : quotient α) (q₂ : quotient β) (f : Π a b, φ ⟦a⟧ ⟦b⟧) : φ q₁ q₂:=
...
open decidable
instance {α : Sort u} {s : setoid α} [d : ∀ a b : α, decidable (a ≈ b)] : decidable_eq (quotient α) :=
λ q₁ q₂ : quotient α,
