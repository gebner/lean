...
void module_mgr::rebuild_rdeps(module_id const & id) {
    for (auto & mod : m_modules) {
        if (mod.second.m_version == m_current_period) continue;
        for (auto & dep : mod.second.m_deps) {
            if (dep.first == id) {
                build_module(mod.first, true, {});
                rebuild_rdeps(mod.first);
                break;
            }
        }
    }
...
public:
    parse_lean_task(std::string const & contents, environment const & initial_env,
        ...
        module_task(task_priority::PARSING_PRIO, task_priority::ROI_PARSING_PRIO, {}),
    ...
    module_info::parse_result execute_core() override {
                ...
                if (std::get<0>(d) == base &&
                    ...
                    mod_info.m_result->get().m_delayed_theorems.for_each(
                            [&] (name const & thm_name, delayed_theorem const & del_thm) {
                                delayed_thms.insert(thm_name, del_thm.m_proof);
                            });
        ...
        if (m_use_snapshots) infom = { info_manager() };
...
public:
    olean_compilation_task(module_info const & mod) :
            module_task(task_priority::ELAB_PRIO, task_priority::ELAB_PRIO, optional<pos_info>()),
            m_mod(mod) {}
...
void module_mgr::build_module(module_id const & id, bool can_use_olean, name_set module_stack) {
    if (m_modules.count(id) && m_modules.at(id).m_version == m_current_period) return;
    ...
    scoped_module_mgr scoped_mod_mgr(this, id);
...
bool module_mgr::is_in_roi(module_id const & mod, optional<pos_info> const &) {
    return m_roi.count(mod) > 0;
}
...
LEAN_THREAD_PTR(scoped_module_mgr, g_scoped_module_mgr);
scoped_module_mgr::scoped_module_mgr(module_mgr * mod_mgr, module_id const & module) : m_mgr(mod_mgr), m_mod(module) {
    m_old = g_scoped_module_mgr;
    g_scoped_module_mgr = this;
}
scoped_module_mgr::~scoped_module_mgr() {
    g_scoped_module_mgr = m_old;
}
module_mgr *get_global_module_mgr() {
    return g_scoped_module_mgr->m_mgr;
}
module_id const & get_global_module_id() {
    return g_scoped_module_mgr->m_mod;
}
