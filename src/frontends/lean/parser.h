...
struct delayed_theorem {
    ...
    name get_name() const { return m_ax_decl.get_name(); }
};
...
struct snapshot {
    ...
    snapshot(environment const & env, std::vector<name> const & sub_buckets, name_map<delayed_theorem> delayed_thms, local_level_decls const & lds,
...
class parser : public abstract_parser {
    ...
    std::vector<name>       m_old_buckets_from_snapshot;
...
public:
    parser(environment const & env, io_state const & ios,
           ...
           optional<info_manager> const & infom = optional<info_manager>());
    ~parser();
