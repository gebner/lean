...
public:
    proof_elaboration_task(environment const & decl_env,
        ...
        module_task(task_priority::ELAB_PRIO, task_priority::ROI_ELAB_PRIO, optional<pos_info>(prov.get_some_pos())),
    ...
    expr execute_core() override {
        ...
        try {
            optional<info_manager> infom; // TODO(gabriel)
            elaborator elab(m_decl_env, m_opts, m_mctx, m_lctx, infom);
...
environment single_definition_cmd_core(parser & p, def_cmd_kind kind, decl_modifiers modifiers, decl_attributes attrs) {
    ...
    auto process = [&](expr val) -> environment {
        ...
        if (kind == Theorem) {
            ...
            env_n = declare_definition(p, elab.env(), kind, lp_names, c_name, type, opt_val, modifiers, attrs, header_pos);
        }
